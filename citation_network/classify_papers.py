...
def generate_links_with_classification(search_results, taxonerd):
    ...
    for paper in tqdm(search_results):
        ...
        cited = [(p['paperId'], {'title': p['title'], 'study_system': classify_paper(p['title'], p['abstract'], taxonerd)}) for p in paper['references'] if p['paperId'] is not None]   
        nodes.append(citing)
        nodes.extend(cited)
...
def main(search_result_path, graph_save_path):
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Classify nodes and format graph')
    
    parser.add_argument('search_result_path', type=str,
                       help='Output from pull_papers.py')
    parser.add_argument('graph_save_path', type=str,
